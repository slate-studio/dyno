...
class Mongodb {
  ...
  getSchema(modelName) {
    ...
    try {
      ...
      schema.static('ObjectId', function() {
        if (this._mockObjectIds && this._mockObjectIds.length) {
          return this._mockObjectIds.shift()
        }

        return require('mongoose').Types.ObjectId()
      })
      schema.static('mockObjectId', function(objectId) {
        if (!this._mockObjectIds) {
          this._mockObjectIds = []
        }

        this._mockObjectIds.push(objectId)
      })
    } catch (e) {
      ...
      this.mongoose.connection.db.collection(collection)
        .find(query).explain((err, explaination) => {
          RequestNamespace.create(namespace, [], () => {
            ...
            log.debug({ collection, method, query, options, index },
              `[mongodb] ${collection}.${method}`, query)
          })
        })
    })
  }
