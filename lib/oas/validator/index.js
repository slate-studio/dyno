...
class Validator {
  ...
  validateParameters(parameters) {
    ...
    for (let name in parametersSpec) {
      ...
      if (parameter.required && parameters[name] === undefined) {
  ...
  validateResponseBody(responseStatus, responseBody, operationId) {
    const response = this.operationSpec.responses[responseStatus]
    ...
    if (response.schema) {
      this.validateEntity(responseBody, 'responseBody', response.schema)
    }
  }
  ...
  checkType(types, value) {
    if (!_.isArray(types)) {
      types = [ types ]
    }

    for (let type of types) {
      if (this[`is${_.upperFirst(type)}`](value)) {
        return true
      }
    }
