...
const requestNamespace = require('./oas/requestNamespace')
...
class Server {
  ...
  createServer() {
    ...
    const Authentication   = _.get(this.config, 'service.Authentication')
    const executionTimeout = _.get(this.config, 'server.executionTimeout', 15000)
    ...
    server.use(cors(this.config.server.cors))

    server.use(requestNamespace.createRequestNamespace)
    server.use(responseTime())
    server.use('/', health)
    server.use(timeout(executionTimeout))
