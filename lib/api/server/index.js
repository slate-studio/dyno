...
class Server {
  ...
  createServerMiddleware(server) {
    ...
    const config = _.get(this.config, 'server.cors', {})
  ...
  readSecrets() {
    ...
    if (kmsKey) {
      ...
      credstash = new Credstash(options)
    }

    const handler = async (secrets) => {
      return _.map(secrets, async (secret, name) => {
        const context = secret.context
        let value     = secret.defaultValue

        if (!context || !value) {
          return Promise.all(handler(secret))
        }

        if (credstash) {
          const query = { name, context }
          value       = await credstash.getSecret(query)
        }
    ...
    return Promise.all(handler(secrets))
  }
