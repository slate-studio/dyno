...
class Pool {
  ...
  async initialize(options) {
    options = Object.assign({}, options, { name: this.name })
    ...
    this.pool = new lb_pool.Pool(http, this.endpoints, options)
  ...
  request(options) {
    return new Promise((resolve, reject) => {
      let data
       ...
       if (options.body) {
         data = new Buffer(options.body)
       }

       this.pool.request(options, data, (lbPoolError, res, body) => {
        if (lbPoolError) {
          const stats = this.pool.stats()
