...
class Validator {
  ...
  validateParameters(parameters) {
    const parametersSpec          = this.operationSpec.parameters
    const parametersKays          = _.keys(parameters)
    const allowableParametersKeys = _.map(parametersSpec, 'name')
    const difference              = _.difference(parametersKays, allowableParametersKeys)
    ...
    for (let parameter of parametersSpec) {
      if (parameter.required) {
        ...
        if (parameter.in == 'body') {
          this.validateEntity(value, name, parameter.schema)
        } else {
          this.validateEntity(value, name, parameter)
        }
      }
    }
  }
  ...
  validateResponseStatus(responseStatus, operationId) {
    if (!this.operationSpec.responses[responseStatus]) {
      throw new Error(`Response status '${responseStatus}' is not specified in the schema for ${operationId} mock`)
    }
  }
    ...
    switch(true) {
      ...
      default:
        if (_.includes(['date', 'date-time'], format)) {
          value = this.getStringDate(value, format)
  ...
  //     if (format == 'date-time' && date.toISOString) {
  //       strDate = date.toISOString()
  //     } else if (format == 'date') {
  //       if (date.utc) {
  //         strDate = date.utc().format('YYYY-MM-DD')
  //       } else if (date instanceof Date) {
  //         strDate = moment(date).utc().format('YYYY-MM-DD')
  //       }
  //     }
  //   }
