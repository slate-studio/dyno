...
const errorParse = error => {
  // NOTE Can push `stack` to the list for dev. env. if needed
  ...
  for(let name in exportedErrorFields) {
...
class InternalServerError extends Error {
  constructor(error) {
    ...
    this.originalError  = errorParse(error)
  }
}
...
class Base {
  ...
  response() {
    ...
    return _.map(this.objects, object => {
      return (object.toResponse ? object.toResponse() : object)
    })
  }
  ...
  error(error) {
    ...
    const response = errorParse(error)
    this.res.status(status).json(response)
  }
...
class Export extends Index {
  ...
  response() {
    const data = _.map(this.objects, object => {
      return (object.toResponse ? object.toResponse() : object)
    })
